...
import com.example.todolistwithcompose.presentor.state.UpdateTaskState
...
class UpdateTaskViewModel(private val taskId: Long, private val appContext: Context) : ViewModel() {
    private val taskDao = TasksDatabase.getInstance(context = appContext).taskDao

    private lateinit var task:Task
    private val _state: MutableStateFlow<UpdateTaskState> = MutableStateFlow(UpdateTaskState.Loading)
    ...
    init {
        viewModelScope.launch(Dispatchers.IO) {
            taskDao.getTaskById(taskId).collect{
                ...
                _state.value = UpdateTaskState.Result(task)
            }
        }
    }

    fun setTitle(title: String) {
        val task = task.copy(title = title)
        _state.value = UpdateTaskState.Result(task)
    }
    ...
    private fun checkTask(task: Task): Boolean {
        return when {
            task.title.isEmpty() -> {
                _state.value = UpdateTaskState.Result(task, errorTitle = true)
                false
            }

            task.content.isEmpty() -> {
                _state.value = UpdateTaskState.Result(task, errorContext = true)
                false
            }
